...
class TestJKSalesAPI:
    ...
    def test_jk_sales_summary_meridian(self, api_server):
        ...
        response = requests.get(
            f"{api_server}/api/jks/sales/Meridian%20Apartments/summary"
        )
        assert response.status_code == 200
        data = response.json()
        assert data["success"] is True
        ...
        if "avg_price" in summary:
            ...
            assert (
                summary["min_price"] <= summary["avg_price"] <= summary["max_price"]
            ), "Price statistics should be logical"

            # Should have reasonable price range for Almaty apartments
            assert 10_000_000 <= summary["min_price"] <= 200_000_000, (
                f"Min price {summary['min_price']} should be reasonable for Almaty"
            )
            assert 10_000_000 <= summary["max_price"] <= 500_000_000, (
                f"Max price {summary['max_price']} should be reasonable for Almaty"
            )
    ...
    def test_jk_sales_analysis_meridian(self, api_server):
        ...
        response = requests.get(
            f"{api_server}/api/jks/sales/Meridian%20Apartments/analysis?discount_percentage=0.15"
        )
        ...
        assert global_stats["min"] <= global_stats["median"] <= global_stats["max"], (
            "Min <= Median <= Max should hold"
        )
        assert global_stats["min"] <= global_stats["mean"] <= global_stats["max"], (
            "Min <= Mean <= Max should hold"
        )
        ...
        for flat_type, stats in flat_type_buckets.items():
            ...
            assert stats["min"] <= stats["median"] <= stats["max"], (
                f"{flat_type}: Min <= Median <= Max should hold"
            )
            assert stats["min"] <= stats["mean"] <= stats["max"], (
                f"{flat_type}: Min <= Mean <= Max should hold"
            )

            logger.info(
                f"  {flat_type}: {stats['count']} sales, avg {stats['mean']:,.0f}₸, median {stats['median']:,.0f}₸"
            )
        ...
        logger.info(
            f"Found {total_opportunities} opportunities across {len(opportunities)} flat types"
        )
        ...
        assert isinstance(flat_type_timeseries, dict), (
            "Flat type timeseries should be a dictionary"
        )

        total_data_points = sum(len(points) for points in flat_type_timeseries.values())
        if total_data_points > 0:
            logger.info(
                f"Historical data points: {total_data_points} across {len(flat_type_timeseries)} flat types"
            )
            for flat_type, points in list(flat_type_timeseries.items())[
                :2
            ]:  # Show first 2 flat types
                if points:
                    logger.info(f"  {flat_type}: {len(points)} data points")
                    for point in points[:2]:  # Show first 2 points per type
                        logger.info(
                            f"    {point['date']}: avg {point['mean_price']:,.0f}₸"
                        )
    ...
    def test_jk_sales_opportunities_meridian(self, api_server):
        ...
        response = requests.get(
            f"{api_server}/api/jks/sales/Meridian%20Apartments/opportunities?discount_percentage=0.15&limit=10"
        )
        assert response.status_code == 200
        data = response.json()
        assert data["success"] is True
        ...
        logger.info(
            f"Found {total_found} total opportunities, returning {len(opportunities)}"
        )
        ...
        for i, opp in enumerate(opportunities[:3]):  # Test first 3 opportunities
            ...
            assert "discount_percentage_vs_median" in opp, (
                f"Opportunity {i} should have discount percentage"
            )
            ...
            assert opp["discount_percentage_vs_median"] >= 0, (
                f"Opportunity {i} discount should be non-negative"
            )

            # Market stats validation
            market_stats = opp["market_stats"]
            assert market_stats["mean_price"] > 0, (
                f"Opportunity {i} market mean should be positive"
            )
            assert market_stats["median_price"] > 0, (
                f"Opportunity {i} market median should be positive"
            )
            assert market_stats["count"] > 0, (
                f"Opportunity {i} market count should be positive"
            )

            logger.info(
                f"  Opportunity {i + 1}: {opp['flat_type']} - {opp['price']:,}₸ ({opp['discount_percentage_vs_median']:.1f}% discount)"
            )
    ...
    def test_jk_sales_analysis_different_discounts(self, api_server):
        ...
        response_10 = requests.get(
            f"{api_server}/api/jks/sales/Meridian%20Apartments/analysis?discount_percentage=0.10"
        )
        assert response_10.status_code == 200, (
            f"10% discount analysis failed: {response_10.text}"
        )
        data_10 = response_10.json()

        # Test with 20% discount
        response_20 = requests.get(
            f"{api_server}/api/jks/sales/Meridian%20Apartments/analysis?discount_percentage=0.20"
        )
        assert response_20.status_code == 200, (
            f"20% discount analysis failed: {response_20.text}"
        )
        data_20 = response_20.json()

        # Count opportunities for both
        opps_10 = sum(
            len(opps) for opps in data_10["current_market"]["opportunities"].values()
        )
        opps_20 = sum(
            len(opps) for opps in data_20["current_market"]["opportunities"].values()
        )
        ...
        if opps_20 <= opps_10:
            logger.info(
                "✅ Higher discount found fewer or equal opportunities (expected)"
            )
        else:
            logger.info(
                f"⚠️ Higher discount found more opportunities ({opps_20} vs {opps_10}) - unexpected behavior"
            )
    ...
    def test_jk_sales_analysis_nonexistent(self, api_server):
        ...
        current_market = data["current_market"]
        assert current_market["global_stats"]["count"] == 0, (
            "Non-existent JK should have 0 sales"
        )
        assert current_market["global_stats"]["mean"] == 0, (
            "Non-existent JK should have 0 mean price"
        )
        assert current_market["global_stats"]["median"] == 0, (
            "Non-existent JK should have 0 median price"
        )
        assert current_market["global_stats"]["min"] == 0, (
            "Non-existent JK should have 0 min price"
        )
        assert current_market["global_stats"]["max"] == 0, (
            "Non-existent JK should have 0 max price"
        )
        ...
        flat_type_buckets = current_market["flat_type_buckets"]
        assert isinstance(flat_type_buckets, dict), (
            "Flat type buckets should be a dictionary"
        )
        assert len(flat_type_buckets) == 0, (
            "Non-existent JK should have no flat type buckets"
        )
