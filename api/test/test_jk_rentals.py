...
class TestJKRentalsAPI:
    ...
    def test_jk_rentals_summary_meridian(self, api_server):
        ...
        response = requests.get(
            f"{api_server}/api/jks/rentals/Meridian%20Apartments/summary"
        )
        assert response.status_code == 200
        data = response.json()
        assert data["success"] is True
        ...
        assert "flat_type_distribution" in summary, (
            "Summary should have flat_type_distribution"
        )

        # Test total rentals
        assert summary["total_rentals"] > 0, (
            "Meridian Apartments should have rental data"
        )
    ...
    def test_jk_rentals_analysis_meridian(self, api_server):
        ...
        response = requests.get(
            f"{api_server}/api/jks/rentals/Meridian%20Apartments/analysis?min_yield_percentage=0.05"
        )
        assert response.status_code == 200
        data = response.json()
        assert data["success"] is True
        ...
        for flat_type, stats in flat_type_buckets.items():
            ...
            logger.info(
                f"  {flat_type}: {stats['count']} rentals, mean yield {stats['mean_yield']:.2f}%"
            )
        ...
        for flat_type, opps in opportunities.items():
            if opps:
                ...
                for i, opp in enumerate(opps[:2]):  # Show first 2
                    ...
                    assert opp["yield_percentage"] >= 0.05, (
                        f"Opportunity yield {opp['yield_percentage']}% should be >= 5%"
                    )
                    logger.info(
                        f"    {i + 1}. Flat {opp['flat_id']}: {opp['price']:,}₸/month (Yield: {opp['yield_percentage']:.2f}%)"
                    )
        ...
        for flat_type, points in timeseries.items():
            if points:
                ...
                for point in points[:2]:  # Show first 2
                    ...
                    logger.info(
                        f"    {point['date']}: {point['count']} rentals, avg {point['mean_rental']:,.0f}₸"
                    )

        logger.info(
            f"✅ Meridian Apartments rental analysis completed with {total_opportunities} opportunities"
        )
    ...
    def test_jk_rentals_opportunities_meridian(self, api_server):
        ...
        response = requests.get(
            f"{api_server}/api/jks/rentals/Meridian%20Apartments/opportunities?min_yield_percentage=0.05&limit=5"
        )
        assert response.status_code == 200
        data = response.json()
        assert data["success"] is True
        ...
        for i, opp in enumerate(opportunities):
            ...
            assert "yield_percentage" in opp, (
                f"Opportunity {i} should have yield_percentage"
            )
            assert "flat_type" in opp, f"Opportunity {i} should have flat_type"
            assert "residential_complex" in opp, (
                f"Opportunity {i} should have residential_complex"
            )

            # Validate opportunity data
            assert opp["price"] > 0, f"Opportunity {i} price should be positive"
            assert opp["yield_percentage"] >= 0.05, (
                f"Opportunity {i} yield {opp['yield_percentage']}% should be >= 5%"
            )
            assert opp["residential_complex"] == "Meridian Apartments", (
                f"Opportunity {i} should be from Meridian Apartments"
            )

            logger.info(
                f"  Opportunity {i + 1}: {opp['flat_type']} - {opp['price']:,}₸/month (Yield: {opp['yield_percentage']:.2f}%)"
            )
    ...
    def test_jk_rentals_listings_meridian(self, api_server):
        ...
        response = requests.get(
            f"{api_server}/api/jks/rentals/Meridian%20Apartments/rentals?limit=20"
        )
        assert response.status_code == 200
        data = response.json()
        assert data["success"] is True
        ...
        for i, rental in enumerate(rentals[:5]):  # Test first 5 rentals
            ...
            assert "residential_complex" in rental, (
                f"Rental {i} should have residential_complex"
            )

            # Validate rental data
            assert rental["price"] > 0, f"Rental {i} price should be positive"
            assert rental["area"] > 0, f"Rental {i} area should be positive"
            assert rental["residential_complex"] == "Meridian Apartments", (
                f"Rental {i} should be from Meridian Apartments"
            )

            # Price should be reasonable for rentals
            assert 50_000 <= rental["price"] <= 2_000_000, (
                f"Rental {i} price {rental['price']} should be reasonable for Almaty rentals"
            )

            logger.info(
                f"  Rental {i + 1}: {rental['flat_type']} - {rental['price']:,}₸/month, {rental['area']}m²"
            )
    ...
    def test_jk_rentals_with_filters(self, api_server):
        ...
        response_type = requests.get(
            f"{api_server}/api/jks/rentals/Meridian%20Apartments/rentals?flat_type=2BR&limit=10"
        )
        ...
        response_price = requests.get(
            f"{api_server}/api/jks/rentals/Meridian%20Apartments/rentals?min_price=100000&max_price=500000&limit=10"
        )
        ...
        for rental in data_price["rentals"]:
            assert 100_000 <= rental["price"] <= 500_000, (
                f"Rental price {rental['price']} should be within filter range"
            )
    ...
    def test_jk_rentals_price_trends_meridian(self, api_server):
        ...
        response = requests.get(
            f"{api_server}/api/jks/rentals/Meridian%20Apartments/price-trends"
        )
        assert response.status_code == 200
        data = response.json()
        assert data["success"] is True
        ...
        if trends:
            ...
            for i, trend in enumerate(trends[:3]):  # Test first 3 trends
                ...
                assert trend["min_price"] <= trend["avg_price"] <= trend["max_price"], (
                    f"Trend {i}: Min <= Avg <= Max should hold"
                )

                logger.info(
                    f"  {trend['date']} - {trend['flat_type']}: {trend['count']} rentals, avg {trend['avg_price']:,.0f}₸"
                )
        else:
            logger.info(
                "No price trend data available (this might be normal if data is recent)"
            )
    ...
    def test_rentals_overview(self, api_server):
        ...
        for jk in top_jks[:3]:  # Test first 3
            ...
            logger.info(
                f"  {jk['residential_complex']}: {jk['rental_count']} rentals, avg {jk['avg_price']:,.0f}₸"
            )
        ...
        for dist in flat_type_dist:
            ...
            logger.info(
                f"  {dist['flat_type']}: {dist['count']} rentals, avg {dist['avg_price']:,.0f}₸"
            )

        logger.info(
            f"✅ Rental market overview: {overall_stats['total_rentals']} rentals across {overall_stats['total_jks']} JKs"
        )
    ...
    def test_jk_rentals_nonexistent(self, api_server):
        ...
        assert data["summary"]["total_rentals"] == 0, (
            "Non-existent JK should have 0 rentals"
        )
    ...
    def test_jk_rentals_analysis_nonexistent(self, api_server):
        ...
        assert data["success"] is True
        assert data["jk_name"] == "NonExistentJK"
        assert data["current_market"]["global_stats"]["count"] == 0, (
            "Non-existent JK should have 0 rental data"
        )
        assert len(data["current_market"]["opportunities"]) == 0, (
            "Non-existent JK should have no opportunities"
        )
