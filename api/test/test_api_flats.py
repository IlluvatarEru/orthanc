...
class TestFlatsAPI:
    ...
    def test_flats_search_by_flat_type(self, api_server):
        ...
        response = requests.get(
            f"{api_server}/api/flats/search", params={"flat_type": "2BR", "limit": 10}
        )
        assert response.status_code == 200
        data = response.json()
        assert data["success"] is True
        ...
        for flat in data["flats"]:
            assert flat["flat_type"] == "2BR", (
                f"Expected flat_type=2BR, got {flat['flat_type']}"
            )
    ...
    def test_flats_search_by_residential_complex(self, api_server):
        ...
        response = requests.get(
            f"{api_server}/api/flats/search",
            params={"residential_complex": "Meridian Apartments", "limit": 10},
        )
        assert response.status_code == 200
        data = response.json()
        assert data["success"] is True
    ...
    def test_flats_search_combined_filters(self, api_server):
        ...
        assert data["success"] is True
        logger.info(
            f"Found {data['count']} flats with flat_type=2BR AND residential_complex='Meridian Apartments'"
        )
        ...
        if data["count"] == 0:
            ...
            response_all = requests.get(
                f"{api_server}/api/flats/search", params={"limit": 50}
            )
            if response_all.status_code == 200:
                ...
                logger.info(
                    f"Available residential complexes: {sorted(residential_complexes)}"
                )
                ...
                meridian_flats = [
                    f
                    for f in all_data["flats"]
                    if "Meridian" in f.get("residential_complex", "")
                ]
                logger.info(
                    f"Flats with 'Meridian' in residential_complex: {len(meridian_flats)}"
                )
    ...
    def test_flats_search_price_filters(self, api_server):
        ...
        response = requests.get(
            f"{api_server}/api/flats/search",
            params={"min_price": 10000000, "max_price": 100000000, "limit": 10},
        )
        assert response.status_code == 200
        data = response.json()
        assert data["success"] is True
    ...
    def test_flats_search_rental_filter(self, api_server):
        ...
        # Test rental flats
        response_rental = requests.get(
            f"{api_server}/api/flats/search", params={"is_rental": True, "limit": 10}
        )
        ...
        response_sales = requests.get(
            f"{api_server}/api/flats/search", params={"is_rental": False, "limit": 10}
        )
    ...
    def test_flats_search_debug_all_data(self, api_server):
        ...
        if data["flats"]:
            ...
            meridian_flats = [
                f
                for f in data["flats"]
                if f.get("residential_complex")
                and "Meridian" in f["residential_complex"]
            ]
            logger.info(
                f"Flats with 'Meridian' in residential_complex: {len(meridian_flats)}"
            )
