...
from analytics.src.jk_rentals_analytics import RentalOpportunity, StatsForFlatType
...
async def get_jk_rentals_analysis(
    ...
    try:
        ...
        logger.error(f"Error getting rental analysis for JK {jk_name}: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_jk_rentals_opportunities(
    ...
    try:
        ...
        logger.error(f"Error getting rental opportunities for JK {jk_name}: {e}")
        raise HTTPException(status_code=500, detail=str(e))
