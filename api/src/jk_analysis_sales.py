...
async def get_jk_list():
    ...
    try:
        ...
        return {"success": True, "jks": jks, "count": len(jks)}
...
async def get_jk_sales_analysis(
    jk_name: str,
    discount_percentage: float = Query(
        0.15, description="Discount percentage threshold for opportunities"
    ),
) -> SalesAnalysisResponse:
    """
    ...
    historical_analysis = HistoricalAnalysis(flat_type_timeseries=historical_points)
...
async def get_jk_sales_opportunities(
    jk_name: str,
    discount_percentage: float = Query(
        0.15, description="Discount percentage threshold"
    ),
