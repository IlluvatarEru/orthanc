...
from analytics.src.jk_rentals_analytics import (
    JKRentalAnalytics,
    analyze_jk_for_rentals,
    StatsForFlatType,
)
from analytics.src.jk_rentals_analytics import (
    RentalYieldStats,
...
class TestJKRentalAnalytics:
    ...
    def test_analyze_jk_for_rentals_success(self):
        ...
        logger.info("Minimum yield percentage: 5%")
        ...
        assert analysis is not None, (
            "Failed to analyze JK - returned None. This indicates the analytics is not working properly."
        )
        assert "current_market" in analysis, "Analysis missing current_market data"
        assert "historical_analysis" in analysis, (
            "Analysis missing historical_analysis data"
        )
        ...
        for flat_type, opps in opportunities.items():
            if opps:  # Only show if there are opportunities
                logger.info(
                    f"{flat_type} opportunities: {[opp.yield_percentage for opp in opps]}"
                )
    ...
    def test_current_rental_market_analysis_structure(self):
        ...
        assert current_market is not None, (
            "Cannot test structure - current_market is None."
        )
        ...
        for flat_type, stats in flat_type_buckets.items():
            logger.info(
                f"  {flat_type}: Mean Yield={stats.mean_yield}, Count={stats.count}"
            )
        ...
        for flat_type, opportunities in opportunities_by_type.items():
            ...
            for i, opp in enumerate(opportunities[:2]):
                logger.info(
                    f"    {i + 1}. Flat {opp.flat_info.flat_id}: {opp.flat_info.price:,}₸/month (Yield: {opp.yield_percentage:.2f}%)"
                )
                ...
                assert (
                    opp.yield_percentage >= 0.05
                )  # Should meet minimum yield threshold

        # Verify that opportunities is a dictionary
        assert isinstance(opportunities_by_type, dict), (
            f"Expected dict, got {type(opportunities_by_type)}"
        )
        ...
        if total_opportunities > 0:
            ...
            if flat_type_with_opps:
                logger.info(
                    f"Testing opportunities structure for {flat_type_with_opps}"
                )
    ...
    def test_historical_rental_analysis_structure(self):
        ...
        logger.info(
            f"Analysis period: {historical.analysis_period[0]} to {historical.analysis_period[1]}"
        )
    ...
    def test_opportunity_detection(self):
        ...
        if len(opportunities_8) <= len(opportunities_3):
            logger.info(
                "✅ Higher yield threshold found fewer or equal opportunities (expected)"
            )
        else:
            logger.info(
                f"⚠️ Higher yield threshold found more opportunities ({len(opportunities_8)} vs {len(opportunities_3)}) - unexpected behavior"
            )

        # Test opportunity structure (only if opportunities exist)
        if opportunities_3:
            for opp in opportunities_3:
                assert opp.yield_percentage >= 0.0, (
                    f"Opportunity yield {opp.yield_percentage}% should be >= 0%"
                )
                ...
                assert isinstance(opp.flat_info, FlatInfo), (
                    "Opportunity should have FlatInfo"
                )
                assert isinstance(opp.stats_for_flat_type, StatsForFlatType), (
                    "Opportunity should have StatsForFlatType"
                )
        else:
            logger.info(
                "No opportunities found with 3% yield - this might be normal if no flats meet the criteria"
            )
    ...
    def test_yield_stats_calculation(self):
        ...
        logger.info(
            f"Calculated stats: Mean={stats.mean_yield}, Median={stats.median_yield}, Min={stats.min_yield}, Max={stats.max_yield}"
        )
    ...
    def test_analysis_data_consistency(self):
        ...
        global_stats = current_market.global_stats
        assert (
            global_stats.min_yield
            <= global_stats.median_yield
            <= global_stats.max_yield
        ), "Min <= Median <= Max should hold"
        assert (
            global_stats.min_yield <= global_stats.mean_yield <= global_stats.max_yield
        ...
        for flat_type, stats in current_market.flat_type_buckets.items():
            assert stats.min_yield <= stats.median_yield <= stats.max_yield, (
                f"{flat_type}: Min <= Median <= Max should hold"
            )
            assert stats.min_yield <= stats.mean_yield <= stats.max_yield, (
        ...
        for opp in all_opportunities:
            assert opp.yield_percentage >= 0.05, (
                f"Opportunity {opp.flat_info.flat_id} yield {opp.yield_percentage}% should be >= 5.0%"
            )
            assert opp.flat_info.price > 0, (
                f"Opportunity {opp.flat_info.flat_id} price should be positive"
            )
    ...
    def test_historical_timeseries_data(self):
        ...
        total_data_points = sum(
            len(timeseries) for timeseries in historical.flat_type_timeseries.values()
        )
        ...
        for flat_type, timeseries in historical.flat_type_timeseries.items():
            for data_point in timeseries:
                ...
                assert data_point.mean_rental > 0, (
                    f"Mean rental should be positive: {data_point.mean_rental}"
                )
                assert data_point.median_rental > 0, (
                    f"Median rental should be positive: {data_point.median_rental}"
                )
                assert data_point.min_rental > 0, (
                    f"Min rental should be positive: {data_point.min_rental}"
                )
                assert data_point.max_rental > 0, (
                    f"Max rental should be positive: {data_point.max_rental}"
                )
                assert data_point.count > 0, (
                    f"Count should be positive: {data_point.count}"
                )
