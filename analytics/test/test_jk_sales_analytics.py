...
from analytics.src.jk_sales_analytics import (
    JKAnalytics,
    analyze_jk_for_sales,
    StatsForFlatType,
)
from analytics.src.jk_sales_analytics import (
    PriceStats,
...
class TestJKAnalytics:
    ...
    def test_analyze_jk_for_sales_success(self):
        ...
        logger.info("Discount percentage: 15%")
        ...
        for flat_type, opps in opportunities.items():
            if opps:  # Only show if there are opportunities
                logger.info(
                    f"{flat_type} opportunities: {[opp.discount_percentage_vs_median for opp in opps]}"
                )
    ...
    def test_current_market_analysis_structure(self):
        ...
        for flat_type, opportunities in opportunities_by_type.items():
            ...
            for i, opp in enumerate(opportunities[:2]):
                logger.info(
                    f"    {i + 1}. Flat {opp.flat_info.flat_id}: {opp.flat_info.price:,}₸ ({opp.discount_percentage_vs_median:.1f}% discount)"
                )
    ...
    def test_opportunity_detection(self):
        ...
        if len(opportunities_20) >= len(opportunities_10):
            logger.info(
                "✅ Higher discount found more or equal opportunities (expected)"
            )
        else:
            logger.info(
                f"⚠️ Higher discount found fewer opportunities ({len(opportunities_20)} vs {len(opportunities_10)}) - this can happen with limited data"
            )
        ...
        if opportunities_10:
            for opp in opportunities_10:
                ...
                assert opp.discount_percentage_vs_median >= 0.0, (
                    f"Opportunity discount {opp.discount_percentage_vs_median}% should be >= 0%"
                )
            ...
            logger.info(
                "No opportunities found with 10% discount - this might be normal if no flats meet the criteria"
            )
    ...
    def test_price_stats_calculation(self):
        ...
        logger.info(
            f"Calculated stats: Mean={stats.mean}, Median={stats.median}, Min={stats.min}, Max={stats.max}"
        )
    ...
    def test_analysis_data_consistency(self):
        ...
        for opp in all_opportunities:
            ...
            assert opp.discount_percentage_vs_median >= 14.0, (
                f"Opportunity {opp.flat_info.flat_id} discount {opp.discount_percentage_vs_median}% should be >= 14.0% (allowing for floating-point precision)"
            )
    ...
    def test_historical_timeseries_data(self):
        ...
        for flat_type, timeseries in historical.flat_type_timeseries.items():
            for data_point in timeseries:
                ...
                assert data_point.mean_price > 0, (
                    f"Mean price should be positive: {data_point.mean_price}"
                )
                assert data_point.median_price > 0, (
                    f"Median price should be positive: {data_point.median_price}"
                )
                assert data_point.min_price > 0, (
                    f"Min price should be positive: {data_point.min_price}"
                )
                assert data_point.max_price > 0, (
                    f"Max price should be positive: {data_point.max_price}"
                )
    ...
    def test_edge_cases(self):
        ...
        try:
            analysis = analytics.analyse_jk_for_sales(
                TEST_JK_NAME, 0.99
            )  # 99% discount
            assert analysis is not None
