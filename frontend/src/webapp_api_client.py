...
from api.src.analysis_objects import (
    SalesAnalysisResponse, RentalAnalysisResponse, FlatTypeAnalysisResponse,
    CurrentMarket, GlobalStats, FlatTypeStats, Opportunity, HistoricalAnalysis, HistoricalPoint,
    RentalCurrentMarket, RentalGlobalStats, RentalFlatTypeStats, RentalHistoricalAnalysis, RentalHistoricalPoint
)
...
class WebappAPIClient:
    ...
    def search_complexes(self, query: str) -> Dict:
        ...
        response = requests.get(f"{self.base_url}/api/complexes/search", params={"q": query}, timeout=5)
        response.raise_for_status()
        return response.json()
    
    def get_complex_info(self, residential_complex_name: str) -> Optional[Dict]:
        """Get specific complex information."""
        response = requests.get(f"{self.base_url}/api/complexes/{residential_complex_name}", timeout=5)
        response.raise_for_status()
        data = response.json()
    ...
    def scrape_complex_data(self, residential_complex_name: str, complex_id: Optional[str] = None, 
                           only_rentals: bool = False, only_sales: bool = False) -> Dict:
        ...
        response = requests.post(f"{self.base_url}/api/complexes/{residential_complex_name}/scrape", json=payload)
        response.raise_for_status()
        return response.json()
    
    def refresh_complex_analysis(self, residential_complex_name: str) -> Dict:
        """Refresh analysis for a complex."""
        response = requests.post(f"{self.base_url}/api/complexes/{residential_complex_name}/refresh")
        response.raise_for_status()
        return response.json()
    ...
    def get_jk_sales_analysis(self, jk_name: str, discount_percentage: float = 0.15) -> SalesAnalysisResponse:
        ...
        if data.get('current_market'):
            ...
            if current_market.get('global_stats'):
                current_market['global_stats'] = GlobalStats(**current_market['global_stats'])
            if current_market.get('flat_type_buckets'):
                for flat_type, stats in current_market['flat_type_buckets'].items():
                    current_market['flat_type_buckets'][flat_type] = FlatTypeStats(**stats)
            if current_market.get('opportunities'):
                for flat_type, opps in current_market['opportunities'].items():
                    current_market['opportunities'][flat_type] = [Opportunity(**opp) for opp in opps]
        ...
        if data.get('historical_analysis'):
            ...
            if hist.get('flat_type_timeseries'):
                for flat_type, points in hist['flat_type_timeseries'].items():
                    hist['flat_type_timeseries'][flat_type] = [HistoricalPoint(**point) for point in points]
            data['historical_analysis'] = HistoricalAnalysis(**hist)
        
        return SalesAnalysisResponse(**data)
    
    def get_jk_rentals_analysis(self, jk_name: str, min_yield_percentage: float = 0.05) -> RentalAnalysisResponse:
        ...
        if data.get('current_market'):
            current_market = data['current_market']
            if current_market.get('global_stats'):
                current_market['global_stats'] = RentalGlobalStats(**current_market['global_stats'])
            if current_market.get('flat_type_buckets'):
                for flat_type, stats in current_market['flat_type_buckets'].items():
                    current_market['flat_type_buckets'][flat_type] = RentalFlatTypeStats(**stats)
            if current_market.get('opportunities'):
                for flat_type, opps in current_market['opportunities'].items():
                    current_market['opportunities'][flat_type] = [Opportunity(**opp) for opp in opps]
        ...
        if data.get('historical_analysis'):
            ...
            if hist.get('flat_type_timeseries'):
                for flat_type, points in hist['flat_type_timeseries'].items():
                    hist['flat_type_timeseries'][flat_type] = [RentalHistoricalPoint(**point) for point in points]
