...
def scrape_sales_flat(krisha_id: str) -> Optional[FlatInfo]:
    ...
    try:
        ...
        logging.info(f"Successfully scraped sales flat {krisha_id} via analytics API")
        return flat_info
...
def scrap_and_save_jk_sales(jk_name: str, max_pages: int = 10, db_path: str = "flats.db") -> int:
    ...
    try:
            ...
            for flat_info in flats:
                try:
                    ...
                    if success:
                        ...
                        logging.warning(f"Failed to save sales flat: {flat_info.flat_id}")
        ...
        logging.error(f"Error during JK sales scraping and saving for {jk_name}: {e}")
        return 0
