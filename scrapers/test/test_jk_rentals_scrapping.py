...
class TestJKRentalsScrapping:
    ...
    def test_scrap_jk_rentals_query_only_1(self):
        ...
        """
        logger.info(f"Testing JK rental scraping (query only) for: {TEST_JK_NAME_1}")

        # Scrape with limited pages to avoid long test times
        max_pages = 2
        flats = scrap_jk_rentals(TEST_JK_NAME_1, max_pages=max_pages)

        # Verify results
        assert isinstance(flats, list), f"Expected list, got {type(flats)}"
        logger.info(f"Found {len(flats)} rental flats for {TEST_JK_NAME_1}")

        # CRITICAL: Test should fail if no flats found - this indicates scraping is not working
        assert len(
            flats) > 0, f"No rental flats found for {TEST_JK_NAME_1}. This indicates the JK rental scraper is not working properly."
    ...
    def test_scrap_jk_rentals_query_only_2(self):
        """
        Test JK rental scraping without database writing for JK "Legenda".
        
        This test verifies that the scraping function can find and scrape
        rental flats for the Legenda JK without saving to database.
        """
        logger.info(f"Testing JK rental scraping (query only) for: {TEST_JK_NAME_2}")

        # Scrape with limited pages to avoid long test times
        max_pages = 2
        flats = scrap_jk_rentals(TEST_JK_NAME_2, max_pages=max_pages)

        # Verify results
        assert isinstance(flats, list), f"Expected list, got {type(flats)}"
        logger.info(f"Found {len(flats)} rental flats for {TEST_JK_NAME_2}")

        # CRITICAL: Test should fail if no flats found - this indicates scraping is not working
        assert len(
            flats) > 0, f"No rental flats found for {TEST_JK_NAME_2}. This indicates the JK rental scraper is not working properly."
        ...
        logger.info("✅ JK rental scraping (query only) test passed for Legenda!")

    def test_scrap_and_save_jk_rentals_query_only_1(self):
        """
        ...
        """
        logger.info(f"Testing JK rental scraping (with database) for: {TEST_JK_NAME_1}")
        ...
        try:
            ...
            assert saved_count > 0, f"No rental flats saved for {TEST_JK_NAME_1}. This indicates the JK rental scraper is not working properly."
            ...
            try:
                ...
                logger.info(f"Database contains {db_count} rental flats for {TEST_JK_NAME_1}")
    ...
    def test_flat_info_structure_validation(self):
        ...
        flats = scrap_jk_rentals(TEST_JK_NAME_1, max_pages=1)

        # CRITICAL: Test should fail if no flats found - this indicates scraping is not working
        assert len(
            flats) > 0, f"No flats found for {TEST_JK_NAME_1}. This indicates the JK rental scraper is not working properly."
