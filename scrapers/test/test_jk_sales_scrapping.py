...
TEST_JK_NAME = "ЖК Астана"  # Known complex in Almaty
...
class TestJKSalesScrapping:
    ...
    def test_scrap_jk_sales_query_only(self):
        """
        ...
        """
        logger.info(f"Testing JK sales scraping (query only) for: {TEST_JK_NAME}")
        
        # Scrape with limited pages to avoid long test times
        max_pages = 2
        flats = scrap_jk_sales(TEST_JK_NAME, max_pages=max_pages)
        
        # Verify results
        assert isinstance(flats, list), f"Expected list, got {type(flats)}"
        logger.info(f"Found {len(flats)} sales flats for {TEST_JK_NAME}")
        
        # CRITICAL: Test should fail if no flats found - this indicates scraping is not working
        assert len(flats) > 0, f"No sales flats found for {TEST_JK_NAME}. This indicates the JK sales scraper is not working properly."
    ...
    def test_scrap_jk_sales_with_database(self):
        """
        ...
        """
        logger.info(f"Testing JK sales scraping (with database) for: {TEST_JK_NAME}")
        ...
        try:
            ...
            assert saved_count > 0, f"No sales flats saved for {TEST_JK_NAME}. This indicates the JK sales scraper is not working properly."
            ...
            try:
                ...
                logger.info(f"Database contains {db_count} sales flats for {TEST_JK_NAME}")
    ...
    def test_flat_info_structure_validation(self):
        ...
        flats = scrap_jk_sales(TEST_JK_NAME, max_pages=1)
        
        # CRITICAL: Test should fail if no flats found - this indicates scraping is not working
        assert len(flats) > 0, f"No flats found for {TEST_JK_NAME}. This indicates the JK sales scraper is not working properly."
    ...
    def test_sales_vs_rental_differences(self):
        ...
        sales_flats = scrap_jk_sales(TEST_JK_NAME, max_pages=1)
        
        # CRITICAL: Test should fail if no flats found - this indicates scraping is not working
        assert len(sales_flats) > 0, f"No sales flats found for {TEST_JK_NAME}. This indicates the JK sales scraper is not working properly."
