...
class TestMigRatesFetching:
    ...
    def test_fetch_eur_rate(self, currency_manager):
        ...
        assert rate > LOWER_BOUND_EUR_KZT, (
            f"EUR rate should be above {LOWER_BOUND_EUR_KZT}, got {rate}"
        )
        assert rate < UPPER_BOUND_EUR_KZT, (
            f"EUR rate should be below {UPPER_BOUND_EUR_KZT}, got {rate}"
        )
    ...
    def test_fetch_usd_rate(self, currency_manager):
        ...
        assert rate > LOWER_BOUND_USD_KZT, (
            f"USD rate should be above {LOWER_BOUND_USD_KZT}, got {rate}"
        )
        assert rate < UPPER_BOUND_USD_KZT, (
            f"USD rate should be below {UPPER_BOUND_USD_KZT}, got {rate}"
        )
    ...
    def test_convert_kzt_to_eur(self, currency_manager):
        ...
        assert converted > test_amount / UPPER_BOUND_EUR_KZT, (
            f"Converted EUR amount should be positive, got {converted}"
        )
        assert converted < test_amount / LOWER_BOUND_EUR_KZT, (
            f"Converted amount should be less than original, got {converted}"
        )
    ...
    def test_convert_kzt_to_usd(self, currency_manager):
        ...
        assert converted > test_amount / UPPER_BOUND_USD_KZT, (
            f"Converted USD amount should be positive, got {converted}"
        )
        assert converted < test_amount / LOWER_BOUND_USD_KZT, (
            f"Converted amount should be less than original, got {converted}"
        )
    ...
    def test_rate_validation(self, currency_manager):
        ...
        for currency in currencies:
            ...
            assert isinstance(rate, (int, float)), (
                f"{currency} rate should be numeric, got {type(rate)}"
            )
    ...
    def test_currency_manager_initialization(self):
        ...
        assert hasattr(manager, "get_latest_rate"), (
            "CurrencyManager should have get_latest_rate method"
        )
        assert hasattr(manager, "convert_kzt_to_eur"), (
            "CurrencyManager should have convert_kzt_to_eur method"
        )
        assert hasattr(manager, "convert_kzt_to_usd"), (
            "CurrencyManager should have convert_kzt_to_usd method"
        )
