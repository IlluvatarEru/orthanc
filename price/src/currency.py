        ...
        try:
            ...
            for match in table_matches:
                ...
                if 100 < mid_rate < 1000:
                    ...
                    logging.info(
                        f"Found {currency} rate: {mid_rate} (buy: {buy_rate}, sell: {sell_rate})"
                    )

            # If we didn't find rates in table format, try alternative patterns
            if not rates:
                logging.info(
                    "Could not find rates in table format, trying alternative patterns."
                )
            ...
            if not rates:
                logging.info(
                    "Could not find rates with specific patterns, trying generic number extraction."
                )
...
def format_price_with_eur(
    kzt_amount: float, currency_manager: CurrencyManager, show_eur: bool = False
) -> str:
    """
...
def format_price_with_currency(
    kzt_amount: float, currency_manager: CurrencyManager, currency: str = "EUR"
) -> str:
    """
