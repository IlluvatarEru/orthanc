...
class TestDatabaseOperations:
    ...
    def test_rental_flat_crud_operations(self, db, sample_rental_flat):
        ...
        success = db.insert_rental_flat(
            sample_rental_flat, url, query_date, sample_rental_flat.flat_type
        )
        ...
        assert (
            inserted_flat.residential_complex == sample_rental_flat.residential_complex
        )
        ...
        update_success = db.update_rental_flat(
            updated_flat, url, query_date, updated_flat.flat_type
        )
        ...
        flat_exists = any(
            flat.flat_id == sample_rental_flat.flat_id for flat in remaining_flats
        )
    ...
    def test_sales_flat_crud_operations(self, db, sample_sales_flat):
        ...
        success = db.insert_sales_flat(
            sample_sales_flat, url, query_date, sample_sales_flat.flat_type
        )
        ...
        assert (
            inserted_flat.residential_complex == sample_sales_flat.residential_complex
        )
        ...
        flat_exists = any(
            flat.flat_id == sample_sales_flat.flat_id for flat in remaining_flats
        )
    ...
    def test_flat_type_classification(self, db):
        ...
        for flat_id, area, expected_type, description in test_cases:
            ...
            rental_flats = db.get_rental_flats_by_date(query_date)
            inserted_flat = next(
                (flat for flat in rental_flats if flat.flat_id == flat_id), None
            )
            assert inserted_flat is not None, f"Flat {flat_id} not found"
            assert inserted_flat.flat_type == expected_type, (
                f"Flat type mismatch for {flat_id}: expected {expected_type}, got {inserted_flat.flat_type}"
            )
    ...
    def test_historical_statistics(self, db):
        ...
        rental_flats = [
            FlatInfo(
                "rental_historical_stats_1",
                100000,
                40.0,
                "1BR",
                "Test Complex historical stats",
                1,
                5,
                2020,
                "Да",
        ...
        stats = db.get_historical_statistics(
            query_date, query_date, "Test Complex historical stats"
        )
        ...
        assert stats["rental_stats"]["min_rental_price"] == 100000, (
            "Incorrect min rental price"
        )
        assert stats["rental_stats"]["max_rental_price"] == 150000, (
            "Incorrect max rental price"
        )
        assert stats["sales_stats"]["min_sales_price"] == 10000000, (
            "Incorrect min sales price"
        )
        assert stats["sales_stats"]["max_sales_price"] == 15000000, (
            "Incorrect max sales price"
        )
    ...
    def test_jk_performance_snapshot(self, db):
        ...
        db.insert_rental_flat(
            rental_flat, "https://krisha.kz/a/show/perf_rental_1", query_date, "1BR"
        )
        ...
        db.insert_sales_flat(
            sales_flat, "https://krisha.kz/a/show/perf_sales_1", query_date, "1BR"
        )

        # Create performance snapshot
        success = db.create_jk_performance_snapshot(
            residential_complex_name, query_date
        )
        assert success, "Failed to create performance snapshot"

        # Verify snapshot was created
        snapshots = db.get_jk_performance_snapshots(
            residential_complex=residential_complex_name
        )
        assert len(snapshots) > 0, "No performance snapshots found"

        snapshot = snapshots[0]
        assert snapshot["residential_complex"] == residential_complex_name, (
            "Incorrect complex name in snapshot"
        )
        ...
        db.conn.execute(
            "DELETE FROM rental_flats WHERE flat_id = ? AND query_date = ?",
            (rental_flat.flat_id, query_date),
        )
        db.conn.execute(
            "DELETE FROM sales_flats WHERE flat_id = ? AND query_date = ?",
            (sales_flat.flat_id, query_date),
        )
        db.conn.execute(
            "DELETE FROM jk_performance_snapshots WHERE residential_complex = ? AND snapshot_date = ?",
            (residential_complex_name, query_date),
        )
        db.conn.commit()
        db.disconnect()
