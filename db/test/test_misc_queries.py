    ...
    def test_get_latest_sales_by_jk(self, db):
        ...
        if recent_sales:
            ...
            for sale in recent_sales:
                ...
                assert time_diff <= timedelta(hours=24), f"Sale {sale['flat_id']} updated_at {updated_at_str} is older than 24 hours (diff: {time_diff})"

            logging.info(f"Found {len(recent_sales)} recent sales for {jk_name}")
        else:
            logging.info(f"No recent sales data found for {jk_name} (updated within 24 hours)")
